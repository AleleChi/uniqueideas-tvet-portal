  ...
  try {
    ...
    await pool.query(`
      ...
      SELECT setval('ideas_beneficiary_id_seq', COALESCE((
        SELECT MAX(CAST(substring(id from '\\d+
...
export class DbRepo {
  ...
  static async getTraineeProfiles(params: {
    ...
    if (search) {
      whereClause += ` AND (
        tp.tvet_id ILIKE $${valIndex} OR 
        b.first_name ILIKE $${valIndex} OR 
        b.last_name ILIKE $${valIndex} OR 
        tp.nin ILIKE $${valIndex} OR 
        tp.bvn ILIKE $${valIndex} OR 
        tp.skill ILIKE $${valIndex} OR 
        tp.guardian_name ILIKE $${valIndex}
      )`;
      values.push(`%${search}%`);
      valIndex++;
    }
    ...
    if (state) {
      whereClause += ` AND tp.state = $${valIndex}`;
      values.push(state);
      valIndex++;
    }

    if (skill) {
      whereClause += ` AND (tp.skill = $${valIndex} OR tp.sector = $${valIndex})`;
      values.push(skill);
      valIndex++;
    }

    if (tsp) {
      whereClause += ` AND tp.tsp = $${valIndex}`;
      values.push(tsp);
      valIndex++;
    }
    ...
    try {
      ...
      const countRes = await pool.query(
        `SELECT COUNT(*) as count 
         ...
         JOIN beneficiaries b ON tp.beneficiary_id = b.id
         ${whereClause}`,
        values
      );
      ...
      const queryStr = `
        ...
        FROM trainee_profiles tp
        JOIN beneficiaries b ON tp.beneficiary_id = b.id
        LEFT JOIN portal_monitoring pm ON tp.beneficiary_id = pm.beneficiary_id
        ${whereClause}
    ...
    try {
      const countRes = await pool.query(
        `SELECT COUNT(*) as count 
         ...
         JOIN trainee_profiles tp ON b.id = tp.beneficiary_id
         ${whereClause}`,
        values
      );
      const total = parseInt(countRes.rows[0].count, 10);
    ...
    try {
      const res = await pool.query(`
        INSERT INTO trainee_attendance (
          ...
          status,
          updated_at
        ) VALUES ($1, $2, $3, $4, COALESCE($5, 'MANUAL'), COALESCE($6, 'PRESENT'), NOW())
    ...
    try {
      const res = await pool.query(`
        INSERT INTO portal_monitoring (
          ...
          last_verified_at,
          updated_at
        ) VALUES ($1, COALESCE($2, TRUE), COALESCE($3, TRUE), COALESCE($4, ''), NOW(), NOW())
        ON CONFLICT (beneficiary_id) DO UPDATE SET
    ...
    try {
      const countRes = await pool.query(
        `SELECT COUNT(*) as count 
         ...
         JOIN beneficiaries b ON pm.beneficiary_id = b.id
         JOIN trainee_profiles tp ON b.id = tp.beneficiary_id
         ${whereClause}`,
        values
      );
      ...
      const queryStr = `
        ...
        JOIN trainee_profiles tp ON b.id = tp.beneficiary_id
        ${whereClause}
        ORDER BY pm.last_verified_at DESC
        LIMIT $${valIndex} OFFSET $${valIndex + 1}
      `;
